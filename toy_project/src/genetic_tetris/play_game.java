		...
		for (int i = 0; i < size; i++) { // 회전
			...
			outter: while (true) {
				...
				for (Point p : temp) {
					...
					if (ny < 0 || ny >= 10 || board[p.x][ny] != 0) { // 못 움직이는 경우
						if (ny < 0)
							idx *= -1;
						else if (ny >= 10)
							break outter;
						else if (board[p.x][ny] != 0) {
							if (idx > 0)
								break outter;
							else
								idx *= -1;
				...
				if (!check) {
					...
					inner: while (true) {
						...
						for (Point p : temp) {
							int ny = p.y + inner_move;

							if (ny < 0 || ny >= 10 || board[p.x][ny] != 0) { // 내린 후 못움직이는 경우
								if (ny < 0)
									inner_idx *= -1;
								else if (ny >= 10)
									break inner;
